...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            # f = [p3_idx, p4_idx, p5_idx] — three backbone layer indices
            # args from YAML = [feature_size, num_layers]
            # parse_model prepends in_channels so BiFPN.__init__ receives:
            #   (in_channels=[...], feature_size=..., num_layers=...)
            in_channels = [ch[x] for x in f]
            args = [in_channels, *args]   # args now = [in_channels, feature_size, num_layers]
            c2 = args[1]                  # output channels = feature_size
        else:
            c2 = ch[f]
