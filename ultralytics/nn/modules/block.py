...
# ---------------------------------------------------------------------------
# BiFPN — Bi-directional Feature Pyramid Network (P3/P4/P5 only)
# Adapted from EfficientDet BiFPN for YOLOv11 integration.
# ---------------------------------------------------------------------------
...
class BiFPNBlock(nn.Module):
    """Single bidirectional FPN pass over three feature levels (P3/P4/P5).
    ...
    Bottom-up path:
        ...
        P5_out = fuse(P5, P5_td, downsample(P4_out))

    Fusion uses fast-normalized attention weights (EfficientDet).

    Args:
        feature_size (int): Channels used at every level.
        epsilon (float): Denominator stabiliser for weight normalisation.
    """

    def __init__(self, feature_size: int, epsilon: float = 1e-4):
        super().__init__()
        self.eps = epsilon
        self.p4_td_conv = _DSConv(feature_size)
        self.p3_td_conv = _DSConv(feature_size)
        self.p4_out_conv = _DSConv(feature_size)
        self.p5_out_conv = _DSConv(feature_size)
        # Learnable fast-norm fusion weights
        self.w_td = nn.Parameter(torch.ones(2, 2))   # 2 weights × 2 fused levels
        self.w_bu = nn.Parameter(torch.ones(3, 2))   # 3 weights × 2 fused levels

    @staticmethod
    def _up(src: torch.Tensor, tgt: torch.Tensor) -> torch.Tensor:
        return F.interpolate(src, size=tgt.shape[-2:], mode="nearest")

    @staticmethod
    def _dn(src: torch.Tensor, tgt: torch.Tensor) -> torch.Tensor:
        return F.adaptive_avg_pool2d(src, output_size=tgt.shape[-2:])

    def forward(self, features: list) -> list:
        """Args: [p3, p4, p5] tensors. Returns: [p3_out, p4_out, p5_out]."""
        p3, p4, p5 = features

        wt = F.relu(self.w_td); wt = wt / (wt.sum(0) + self.eps)
        wb = F.relu(self.w_bu); wb = wb / (wb.sum(0) + self.eps)

        # Top-down
        p5_td = p5
        p4_td = self.p4_td_conv(wt[0, 0] * p4 + wt[1, 0] * self._up(p5_td, p4))
        p3_td = self.p3_td_conv(wt[0, 1] * p3 + wt[1, 1] * self._up(p4_td, p3))

        # Bottom-up
        p3_out = p3_td
        p4_out = self.p4_out_conv(
            wb[0, 0] * p4 + wb[1, 0] * p4_td + wb[2, 0] * self._dn(p3_out, p4)
        )
        p5_out = self.p5_out_conv(
            wb[0, 1] * p5 + wb[1, 1] * p5_td + wb[2, 1] * self._dn(p4_out, p5)
        )
...
class BiFPN(nn.Module):
    ...
    Accepts backbone feature maps with arbitrary channel widths, projects each
    to ``feature_size`` channels with a 1×1 Conv, then stacks
    ``num_layers`` rounds of :class:`BiFPNBlock` bidirectional fusion.

    Args:
        in_channels (list[int]): Backbone channels for [P3, P4, P5].
                                 ``parse_model`` sets this automatically.
        feature_size (int): Inner channel width (same for all BiFPN levels).
        num_layers (int): Number of BiFPNBlock rounds to stack.
        epsilon (float): Denominator stabiliser for weight normalisation.

    Input:  list of three tensors ``[p3, p4, p5]``.
    Output: list of three tensors ``[p3_out, p4_out, p5_out]``,
            each with ``feature_size`` channels.
    """
    ...
    def __init__(
        ...
        super().__init__()
        assert len(in_channels) == 3, "BiFPN expects exactly 3 input channel sizes."

        def _proj(c_in: int) -> nn.Sequential:
            return nn.Sequential(
                nn.Conv2d(c_in, feature_size, 1, bias=False),
                nn.BatchNorm2d(feature_size, momentum=0.01, eps=1e-3),
                nn.ReLU(inplace=True),
            )

        self.proj = nn.ModuleList([_proj(c) for c in in_channels])
        self.bifpn = nn.Sequential(*[BiFPNBlock(feature_size, epsilon) for _ in range(num_layers)])

    def forward(self, features: list) -> list:
        """Args: [p3, p4, p5]. Returns: [p3_out, p4_out, p5_out]."""
        features = [proj(f) for proj, f in zip(self.proj, features)]
